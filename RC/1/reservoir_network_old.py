...
class ReservoirNetWork:
    ...
    def __init__(self, inputs, target_data, num_input_nodes, num_reservoir_nodes, num_output_nodes, leak_rate=0.1, activator=np.tanh):
        ...
        self.log_reservoir_nodes = np.array([np.zeros(num_reservoir_nodes)]).T # reservoir層のノードの状態を記録
    ...
    def _get_next_reservoir_nodes(self, input, current_state):
        ...
        next_state = ((1 - self.leak_rate) * current_state).reshape(self.num_reservoir_nodes, 1)
        a = (np.array([input]) @ self.weights_input).reshape(self.num_reservoir_nodes, 1)
        b = (current_state @ self.weights_reservoir).reshape(self.num_reservoir_nodes, 1)
        next_state += self.leak_rate * (a + b)
    ...
    def train(self, lambda0=0.1):
        for input in self.inputs:
            current_state = np.array(self.log_reservoir_nodes)
